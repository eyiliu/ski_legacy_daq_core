...
#include <udpsupp.h>
#include <windows.h>
#include <mmsystem.h>  //’‚∏ˆ∫Õ…œ√Ê“ª–– «∂‡√ΩÃÂø‚£¨“˝”√’‚¡Ω∏ˆø‚£¨»ª∫Û”√PlaySound∫Ø ˝≤•∑≈wav∏Ò Ω
#include <ansi_c.h>
#include <formatio.h>
#include <rs232.h>
#include <utility.h>
	...
	switch (event)
	{
		...
		//	GetPanelHandleFromTabPage(mainHandle,Main_Cmd,0,&usbHandle);
			...
			if(status==VI_SUCCESS)
				InsertTextBoxLine(mainHandle,Main_TEXTBOX,-1,"≈‰÷√≥…π¶£°");  
			else
			{
				MessagePopup("¥ÌŒÛ","∑¢ÀÕCOM≈‰÷√√¸¡Ó ß∞‹£°");
				return(-1);
			}
			break;
	}
		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			if(COM_Status==0)
			{
				MessagePopup("¥ÌŒÛ","Œ¥¥Úø™COM…Ë±∏£°");
				return(-1);
			}
			FileSelectPopup ("", "", "dat", "—°‘ÒTA≈‰÷√Œƒº˛µƒ¬∑æ∂", VAL_OK_BUTTON, 0, 0, 1, 0, ConfigFile);  
			SetCtrlVal(mainHandle,Main_TA_ConfigFile,ConfigFile);
			if((fp_read=fopen(ConfigFile,"r"))==NULL)
			{
				MessagePopup("¥ÌŒÛ","¥Úø™Œƒº˛ ß∞‹£°");
				return(-1);
			}
			if(Monitor_Status==1)
			{
				...
				SetCtrlVal(mainHandle, Main_TEXTBOX, "πÿ±’“£≤‚œﬂ≥Ã..\n");
			...
			while(!feof(fp_read))
			{
				...
				{
					MessagePopup("¥ÌŒÛ","≈‰÷√Œƒº˛”–ŒÛ£°");
					return(-1);
				}
				SendComData(1, TempBuffer);
				Sleep(15);
			}
			SetCtrlVal(mainHandle,Main_TEXTBOX,"TA√¸¡Ó≈‰÷√≥…π¶...\n");
			if(flag==1)
			{
				if(COM_Status==0)
				{
					MessagePopup("¥ÌŒÛ","Œ¥¡¨Ω”COM…Ë±∏£°");
					Monitor_Status=0;
					return(-1);
				}
				CmtScheduleThreadPoolFunction(DEFAULT_THREAD_POOL_HANDLE, MonitorFunction, &panel, &threadID4);
				SetCtrlVal(mainHandle,Main_TEXTBOX,"ø™∆Ù“£≤‚œﬂ≥Ã...\n");
		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	{
		case EVENT_COMMIT:
			DirSelectPopup ("E:\\Work_File\\CEPC\\Data", "—°‘ÒŒƒº˛±£¥Ê¬∑æ∂", 1, 1, WrDataPath);   //E:\Work_File\CEPC\Data
			datasaveflag=1;
...
/*			sprintf(FILE_CMD_LOG, "%s//CMD_LOG_1.txt", WrDataPath);
			fp_cmd_log_1 = OpenFile(FILE_CMD_LOG, VAL_WRITE_ONLY, VAL_APPEND, VAL_ASCII  );
			sprintf(FILE_CMD_LOG, "%s//CMD_LOG_2.txt", WrDataPath);
			fp_cmd_log_2 = OpenFile(FILE_CMD_LOG, VAL_WRITE_ONLY, VAL_APPEND, VAL_ASCII  );
			sprintf(FILE_CMD_LOG, "%s//CMD_LOG_3.txt", WrDataPath);
			fp_cmd_log_3 = OpenFile(FILE_CMD_LOG, VAL_WRITE_ONLY, VAL_APPEND, VAL_ASCII  );
			sprintf(FILE_CMD_LOG, "%s//CMD_LOG_4.txt", WrDataPath);
			fp_cmd_log_4 = OpenFile(FILE_CMD_LOG, VAL_WRITE_ONLY, VAL_APPEND, VAL_ASCII  );
			fp_text_log = OpenFile(FILE_TEXT_BOX,VAL_WRITE_ONLY, VAL_APPEND,VAL_ASCII  );*/

		//	WriteFile(fp_cmd_log, timemsg, File_Length ) ;
			break;
	}
	return 0;
}
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			for(i=2;i<16;i++)
			{
				...
				sprintf(TempMessage,"∑¢ÀÕ¥•∑¢—”≥Ÿ÷µ0x%02xµΩFEE %s...\n",TrigDelay,FEENAME[i]);
				SetCtrlVal(mainHandle,Main_TEXTBOX,TempMessage);
			}
			if(flag==1)
			{
		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			COM_Status=1;
			SetCtrlVal(mainHandle,Main_TEXTBOX,"COM…Ë±∏“—¡¨Ω”...\n");
			FlushOutQ(1);
			...
			{	
				...
				{
					Fmt(TempMessage,"%s<%s%s%s","FEE",FEENAME[i],"ªÿª∑≤‚ ‘...\n");
					SetCtrlVal(mainHandle,Main_TEXTBOX,TempMessage);
				}
			/*	if ( (ResetFPGA(1,FEEID[i]))==-1)
				...
				{
					Fmt(TempMessage,"%s<%s%s%s","FEE",FEENAME[i],"FPGA¬ﬂº≠∏¥Œª...\n");
					SetCtrlVal(mainHandle,Main_TEXTBOX,TempMessage);
				}		  */
				...
				{
					Fmt(TempMessage,"%s<%s%s%s","FEE",FEENAME[i],"¥•∑¢º∆ ˝«Â¡„...\n");
					SetCtrlVal(mainHandle,Main_TEXTBOX,TempMessage);
				}
				...
				{
					Fmt(TempMessage,"%s<%s%s%s","FEE",FEENAME[i],"¥•∑¢Ω” ’πÿ±’...\n");
					SetCtrlVal(mainHandle,Main_TEXTBOX,TempMessage);
				}  
			}
		    Set_Current_Thr();
		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		...
		/*	if(FEE[0]+FEE[1]+FEE[2]+FEE[3]+FEE[4]+FEE[5]+FEE[6]+FEE[7]+FEE[8]+FEE[9]+FEE[10]+FEE[11]+FEE[12]+FEE[13]+FEE[14]+FEE[15]==0)
			...
			if (status < VI_SUCCESS)
			{
				MessagePopup ("¥ÌŒÛ", "¥Úø™USB…Ë±∏ ß∞‹£°");
				SetCtrlVal(mainHandle,Main_USB_Status,0);
				USB_Status=0;
				return -1;
			}
			status = viFindRsrc(defaultRM, "USB[0-9]::0x04B4::0x1004::NI-VISA-[0-9]000[0-9]::RAW", VI_NULL, VI_NULL, desc);		//À—À˜…Ë±∏£¨Ωˆ∆•≈‰VID∫ÕPID
			if (status < VI_SUCCESS)
			{
				status = viFindRsrc(defaultRM, "USB[0-9]::0x04B4::0x1004::NI-VISA-[0-9]::RAW", VI_NULL, VI_NULL, desc);
				if(status<VI_SUCCESS)
				{
					MessagePopup ("¥ÌŒÛ", "USB…Ë±∏≤ª¥Ê‘⁄£°");
					SetCtrlVal(mainHandle,Main_USB_Status,0);
					USB_Status=0;
					return -1;
				}
		...
		/*	GetCurrentDateTime(&CurTime);
			...
			Time_Length = strlen ( timemsg ) ;
			WriteFile(fp_text_log, timemsg, Time_Length ) ;	   */
			SetCtrlVal(mainHandle,Main_TEXTBOX,"USB…Ë±∏“—¡¨Ω”...\n"); 
			sprintf(text_log,"USB…Ë±∏“—¡¨Ω”...\n");
		/*	Log_Length = strlen(text_log);
			WriteFile(fp_text_log, text_log, Log_Length );		*/
		...
		/*	USB_Status=1;
			...
			{
			   	MessagePopup ("¥ÌŒÛ", "π˛π˛π˛£¨ÕÊÀ¿ƒ„£°«Î”√»»∑Áª˙º”»»USB–æ∆¨£¨ªÚ’ﬂ¡Ω∏ˆµÁ‘¥“ª∆ø™£® ÷ÀŸ“™øÏ£©£°");
				return -1;
			}
			else
			{
				...
				Time_Length = strlen ( timemsg ) ;
				WriteFile(fp_text_log, timemsg, Time_Length ) ;
				sprintf(text_log,"Sub_DAQœµÕ≥∏¥Œª...\n");
				SetCtrlVal(mainHandle,Main_TEXTBOX,text_log);
				Log_Length = strlen(text_log);
				WriteFile(fp_text_log, text_log, Log_Length );
				
			}
				//	SetCtrlVal(mainHandle,Main_TEXTBOX,"Sub_DAQœµÕ≥∏¥Œª...\n");   
			     
			CommandWord[0] = 0x03;
			CommandWord[1] = 0x3f;
			if(SendUsbData(instr,CommandWord)==-1)
			{
			   	MessagePopup ("¥ÌŒÛ", "∑¢ÀÕUSB≈‰÷√√¸¡Ó ß∞‹£°£°2");
				return -1;
			}
			else
			    SetCtrlVal(mainHandle,Main_TEXTBOX,"À˘”–Õ®µ¿—°‘ÒŒ™÷˜∑›...\n");
			for(i=0;i<16;i++)
			{
				if(FEE[i]==0)
					continue;
				...
				{
					MessagePopup("¥ÌŒÛ","∑¢ÀÕUSB≈‰÷√√¸¡Ó ß∞‹£°3");
					return(-1);
				}
				else
				{
					sprintf(TempMessage,"FEE%dπ“‘ÿŒ™FEE %s\n",i+1,FEENAME[i]);
					SetCtrlVal(mainHandle,Main_TEXTBOX,TempMessage);
				}
			} 
			CommandWord[0] = 0x02;
			...
			if(SendUsbData(instr,CommandWord)==-1)
			{
			    ...
			    SetCtrlVal(mainHandle,Main_TEXTBOX,"πÿ±’À˘”–Õ®µ¿...\n"); 
			CommandWord[0] = 0x0a;
			CommandWord[1] = 0xaf;
			if(SendUsbData(instr,CommandWord)==-1)
			{
				...
				SetCtrlVal(mainHandle,Main_TEXTBOX,"πÿ±’À˘”–√¸¡Ó∑µªÿÕ®µ¿...\n");
			for(i=0;i<16;i++)
			{
				if(FEE[i]==0)
					continue;
				...
				{
					MessagePopup("¥ÌŒÛ","∑¢ÀÕUSB≈‰÷√√¸¡Ó ß∞‹£°6i");
					return(-1);
				}
				else
				{
					sprintf(TempMessage,"FEE %s√¸¡Ó∑µªÿÕ®µ¿¥Úø™...\n",FEENAME[i]);
					SetCtrlVal(mainHandle,Main_TEXTBOX,TempMessage);
				}
			}		  */
		/*	viRead(instr,TmpData,512,&n);
			...
			viRead(instr,TmpData,512,&n);
			Sleep(10);
			SetCtrlVal(mainHandle,Main_TEXTBOX,"«Âø’Buffer...\n");	   */
		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	{
		case EVENT_COMMIT:
			MessagePopup ("±ß«∏", "‘› ±≤ª÷ß≥÷»Ìº˛∏¥Œª£¨«Î∞¥∏¥Œªº¸U72£°");
			break;
	}
	return 0;
}
         ...
         if(SendUsbData(instr, CommandBuffer) ==-1)
      		{
          	 ...
          	 //printf("Command[1] = %02x, Command[0] = %02x\n",CommandBuffer[1],CommandBuffer[0]);
			       SetCtrlVal(mainHandle,Main_TEXTBOX,"…Ë÷√Œ™ƒ⁄¥•∑¢ƒ£ Ω\n");
		      }
         ...
         if(SendUsbData(instr, CommandBuffer) ==-1)
      		{
          	 ...
          	 //printf("Command[1] = %02x, Command[0] = %02x\n",CommandBuffer[1],CommandBuffer[0]);
             sprintf(TempMessage,"…Ë÷√ƒ⁄¥•∑¢º‰∏ÙŒ™ %d ms\n",InternalTime);
			       SetCtrlVal(mainHandle,Main_TEXTBOX,TempMessage);
		      }
         ...
         if(SendUsbData(instr, CommandBuffer) ==-1)
      		{
          	 ...
          	 //printf("Command[1] = %02x, Command[0] = %02x\n",CommandBuffer[1],CommandBuffer[0]);
			       SetCtrlVal(mainHandle,Main_TEXTBOX,"…Ë÷√Œ™Õ‚¥•∑¢ƒ£ Ω£¨¥•∑¢–≈∫≈Œ™500nsøÌ£¨∏ﬂµÁ∆Ω”––ßµƒ–≈∫≈\n");
		      }
					break;
				case 3:
          CommandBuffer[1] = 0xaa;
         ...
         if(SendUsbData(instr, CommandBuffer) ==-1)
      		{
          	 ...
          	 //printf("Command[1] = %02x, Command[0] = %02x\n",CommandBuffer[1],CommandBuffer[0]);
			       SetCtrlVal(mainHandle,Main_TEXTBOX,"…ËŒ™◊‘¥•∑¢°∞ªÚ°±ƒ£ Ω£¨4∏ˆTAœ‡ªÚ\n");
		      }
					break;
				 case 4:
          CommandBuffer[1] = 0xaa;
         ...
         if(SendUsbData(instr, CommandBuffer) ==-1)
      		{
          	 ...
          	 //printf("Command[1] = %02x, Command[0] = %02x\n",CommandBuffer[1],CommandBuffer[0]);
			       SetCtrlVal(mainHandle,Main_TEXTBOX,"…ËŒ™◊‘¥•∑¢°∞”Î°±ƒ£ Ω\n");
		      }
					break;
					
				default:
					MessagePopup("¥ÌŒÛ","≤ª√˜µƒ¥•∑¢ƒ£ Ω£°");
					return(-1);
      ...
      if(SendUsbData(instr, CommandBuffer) ==-1)
      		{
          	 ...
          	 //printf("Command[1] = %02x, Command[0] = %02x\n",CommandBuffer[1],CommandBuffer[0]);
			       SetCtrlVal(mainHandle,Main_TEXTBOX,"ø™∆Ù¥•∑¢\n");
		      }
		...
		//	Sleep(15);
			...
			GetData_Status=1; 
			SetCtrlVal(mainHandle,Main_TEXTBOX,"ø™ º ˝æ›≤…ºØ...\n");
			...
			SetCtrlVal(mainHandle,Main_TEXTBOX,"ø™∆Ùº‡≤‚...\n");  
			CmtScheduleThreadPoolFunction (DEFAULT_THREAD_POOL_HANDLE, DataAcq, &mainHandle, &threadID1);
			SetCtrlVal(mainHandle,Main_TEXTBOX,"ø™∆Ù ˝æ›≤…ºØœﬂ≥Ã...\n");
      ...
      if(SendUsbData(instr, CommandBuffer) ==-1)
      		{
          	 ...
          	 //printf("Command[1] = %02x, Command[0] = %02x\n",CommandBuffer[1],CommandBuffer[0]);
			       SetCtrlVal(mainHandle,Main_TEXTBOX,"πÿ±’¥•∑¢\n");
		      }
			GetData_Status=0;
			Sleep(100); 
			SetCtrlVal(mainHandle,Main_Acq_Status,0);
			SetCtrlVal(mainHandle,Main_TEXTBOX,"Ω· ¯≤…ºØ√¸¡Ó∑¢≥ˆ...\n");
...
//			TrigDelay = 0x1b;		modified by siyuan 20140807
			...
			for(i=0;i<16;i++)
			{
				...
				sprintf(TempMessage,"…Ë÷√FEE %sµƒ¥•∑¢—”≥Ÿ÷µŒ™%d...\n",FEENAME[i],TrigDelay);
				SetCtrlVal(mainHandle,Main_TEXTBOX,TempMessage);
			}
  ...
  char TempMesg[300];
	...
	switch (event)
	{
      ...
      if(SendUsbData(instr, CommandBuffer) ==-1)
			...
			{
    			sprintf(TempMesg,"…Ë÷√øÃ∂»ƒ£ ΩŒ™0x%02x \n",CommandBuffer[0]);
          	//	printf("Command[1] = %02x, Command[0] = %02x\n",CommandBuffer[1],CommandBuffer[0]);
      ...
      if(SendUsbData(instr, CommandBuffer) ==-1)
			...
			else
			{
          		...
          		printf("Command[1] = %02x, Command[0] = %02x\n",CommandBuffer[1],CommandBuffer[0]); 
			    SetCtrlVal(mainHandle,Main_TEXTBOX,"ÕÍ≥…TA≈‰÷√\n");
		  }
...
//int CVICALLBACK TA_Test_Func (int panel, int control, int event,		 //done
//		void *callbackData, int eventData1, int eventData2)
//{
//	char WrDataFile[300];
//	char timemsg[20];
...
//			Cmd2DAQ_ChooseTrigMode(instr, 1);
//			Cmd2DAQ_StartAcq(instr);
//			Cmd2DAQ_EnableTrig(instr);
//
/////////////////////////////debug///////////////////////////////////      
////      for ( thr = 0x300; thr < 0x500; thr=thr + 10)
////      {
...
////				else
////				{
////					sprintf(TempMessage,"FEEºƒ¥Ê∆˜∏¥Œª\n");
////					SetCtrlVal(mainHandle,Main_TEXTBOX,TempMessage);
////				}
////				for ( i = 0; i <4 ; i++ )
////					TA_Thr(1, FEEID[0], i, thr );
////				
////				TA_DAC(1,FEEID[0],0x40) ;
////					
////				if((TA_Self_Test(1,FEEID[0],80))==-1)
////				{
////					MessagePopup("¥ÌŒÛ","∑¢ÀÕCOM≈‰÷√√¸¡Ó ß∞‹£°");
////					return(-1);
////				}
////				else
////				{
////					sprintf(TempMessage,"Choose Channel %d\n",80);
////					SetCtrlVal(mainHandle,Main_TEXTBOX,TempMessage);
////				}
////				Sleep(10) ;
...
////			  sprintf(TempMessage,"Top5 thr: %x,", thr);
////			  SetCtrlVal(mainHandle,Main_TEXTBOX,TempMessage);
////			  Length = strlen(TempMessage);
////			  WriteFile(ta_fp, TempMessage, Length);
////
...
//				}
//				for ( i = 0; i <4 ; i++ )
//					TA_Thr(1, FEEID[0], i, 0x900);
//				
//				TA_DAC(1,FEEID[0],0x40) ;
//					
//				if((TA_Self_Test(1,FEEID[0],Channel))==-1)
//				{
//					MessagePopup("¥ÌŒÛ","∑¢ÀÕCOM≈‰÷√√¸¡Ó ß∞‹£°");
//					return(-1);
//				}
...
//					else
//						Hit_Num1 = Response[27] ;
//					sprintf(TempMessage,"Top5 Channel %d,", Channel-64);
//					SetCtrlVal(mainHandle,Main_TEXTBOX,TempMessage);
//					Length = strlen(TempMessage);
//					WriteFile(ta_fp, TempMessage, Length);
//				}
...
//					sprintf(TempMessage,"Btm5 Channel %d,", Channel-96);
//					SetCtrlVal(mainHandle,Main_TEXTBOX,TempMessage);
//					Length = strlen(TempMessage);
//					WriteFile(ta_fp, TempMessage, Length);
//				}
...
//					sprintf(TempMessage,"Top8 Channel %d,", Channel-128);
//					SetCtrlVal(mainHandle,Main_TEXTBOX,TempMessage);
//					Length = strlen(TempMessage);
//					WriteFile(ta_fp, TempMessage, Length);
//				}
...
//					sprintf(TempMessage,"Btm8 Channel %d,", Channel-160);
//					SetCtrlVal(mainHandle,Main_TEXTBOX,TempMessage);
//					Length = strlen(TempMessage);
//					WriteFile(ta_fp, TempMessage, Length);
//				}
	...
	switch (event)
	{
	 ...
	 //  printf("T5 = %3f, *DACcode = %03x \n",T5,(DACcode)); 
      ...
      if(SendUsbData(instr,CommandBuffer) ==-1)
			...
			{
    			sprintf(TempMesg,"…Ë÷√TA1µƒ„–÷µŒ™%3d fC \n",(int)T5);
	...
	switch (event)
	{
	 ...
	 //  printf("T5 = %3f, *DACcode = %03x \n",T5,(DACcode)); 
      ...
      if(SendUsbData(instr,CommandBuffer) ==-1)
			...
			{
    			sprintf(TempMesg,"…Ë÷√TA2µƒ„–÷µŒ™%3d fC \n",(int)B5);
	...
	switch (event)
	{
	 ...
	 //  printf("T5 = %3f, *DACcode = %03x \n",T5,(DACcode)); 
      ...
      if(SendUsbData(instr,CommandBuffer) ==-1)
			...
			{
    			sprintf(TempMesg,"…Ë÷√TA3µƒ„–÷µŒ™%3d fC \n",(int)T8);
        ...
        if(FEE[i] == 0)
        ...
        else
      	{
      		SetTHR(1,FEEID[i],DACvalue,type);
        		    sprintf(tempmessage,"Ω´%sµƒB8„–÷µ…ËŒ™:%.0fmV\n",FEENAME[i],B8);
			SetCtrlVal(mainHandle,Main_TEXTBOX,tempmessage);
		}
      }
			break;
		...
		case EVENT_COMMIT:
			...
			if(Cmd2DAQ_CMD_Sel(instr,CMD_Sel)==-1)
				...
				else
				{
					if(CMD_Sel== 0)
					{
						sprintf(TempMessage,"…Ë÷√CMD√¸¡ÓÕ®π˝÷˜Õ®µ¿∑¢ÀÕ...\n");
						SetCtrlVal(mainHandle,Main_TEXTBOX,TempMessage);
					}
					else
					{
...
/*int CVICALLBACK Duty_Mode (int panel, int event, void *callbackData,
						   int eventData1, int eventData2)
{
	switch (event)
	{
							  ...
							  void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_TIMER_TICK:
			...
			MessagePopup("Ã·–—","2–° ±“—µΩ,«Îº«¬ºµÁ¡˜µ»πÿº¸ ˝æ›!");	
			Beep();
...
/*int CVICALLBACK Mask_All (int panel, int control, int event,
						  void *callbackData, int eventData1, int eventData2)
{
	unsigned char TempBuffer[2];
	unsigned char TempMessage[100];
	...
	switch (event)
	{
		...
		//	SetCtrlVal(Mask_Handle, Panel_Mask_Ch1, 1);
		
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch1,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch2,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch3,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch4,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch5,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch6,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch7,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch8,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch9,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch10,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch11,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch12,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch13,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch14,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch15,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch16,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch17,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch18,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch19,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch20,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch21,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch22,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch23,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch24,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch25,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch26,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch27,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch28,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch29,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch30,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch31,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch32,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch33,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch34,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch35,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch36,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch37,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch38,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch39,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch40,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch41,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch42,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch43,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch44,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch45,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch46,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch47,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch48,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch49,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch50,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch51,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch52,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch53,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch54,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch55,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch56,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch57,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch58,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch59,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch60,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch61,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch62,Maks_All_Sig);
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch63,Maks_All_Sig);	 
			   SetCtrlVal(Mask_Handle,Panel_Mask_Ch64,Maks_All_Sig);
